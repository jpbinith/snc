...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('rviz_config_path',
        ...
        # Node reacting to detected objects
        Node(
            package='snc',
            executable='node_start',
            name='node_start',
            output='screen',
            remappings=[('/objectsStamped', '/objectsStamped')],
        ),

        # Wall-following (maze exploration)
        Node(
            package='snc',
        ...
        #  Track Home node (included as a separate launch file)
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(snc_launch_dir, 'track_home.launch.py')
            )
        )
    ])